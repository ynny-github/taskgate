...
- **Type consistency:** `cmd.Run(args []string, stdout, stderr io.Writer) int` is the single embeddable entry point, referenced consistently in Tasks 2, 3, and the design doc. `show.ExitError` is the only sentinel translated in `cmd.Run`. Render shape (path `\t` summary) matches `render_human.go` exactly.
